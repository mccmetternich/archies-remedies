...
// Comprehensive male names list (500+ names)
...
// Protected names (gender-neutral/female-leaning - do NOT filter)
...
function cleanPhone(phone: string): string | null {
  ...
  // Strip leading apostrophe from Shopify exports
  phone = phone.replace(/^'/, '');
...
function getAreaCode(phone: string): string | null {
  ...
  // For US numbers: +1XXXXXXXXXX, area code is positions 1-3 (after country code)
  if (digits.length === 11 && digits.startsWith('1')) {
    return digits.substring(1, 4);
  }
  // For 10-digit numbers, area code is first 3 digits
  if (digits.length === 10) {
...
// Map CSV columns to our contact fields
function mapCSVRow(headers: string[], row: string[]): ParsedRow | null {
  ...
  // Find phone column (prefer 'phone' over 'default address phone')
  let phone = '';
  if (data['phone']) {
    phone = data['phone'];
    ...
    // Look for any column containing 'phone'
    for (const key in data) {
