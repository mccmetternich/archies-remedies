...
import { ArrowLeft, MessageSquare, Upload, Tag, Star, Plus, Trash2, GripVertical, Check, X, Edit2, Loader2, AlertCircle } from 'lucide-react';
...
export default function ProductReviewsPage() {
  ...
  const saveKeywords = async () => {
    ...
    try {
      await fetch('/api/admin/review-keywords', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ keywords, productId }),
      });
      setHasChanges(false);
    } catch (error) {
      console.error('Failed to save:', error);
  ...
  return (
    <div className="space-y-6">
      ...
      {activeTab === 'import' && (
        <ImportTab productId={productId} onImport={fetchData} />
      )}
      {activeTab === 'tags' && (
        <TagsTab
          keywords={keywords}
...
function ImportTab({ productId, onImport }: { productId: string; onImport: () => void }) {
  ...
  const handleImport = async () => {
    ...
    try {
      const res = await fetch('/api/admin/reviews/import', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ csvData, productId, mode }),
      });
  ...
  const handleAddKeyword = async () => {
    ...
    try {
      const res = await fetch('/api/admin/review-keywords', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ keyword: newKeyword.trim(), productId }),
      });
